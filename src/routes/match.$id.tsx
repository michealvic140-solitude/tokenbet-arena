...
export const Route = createFileRoute("/match/$id")({
  head: () => ({ meta: [{ title: "Match — LomitaBet" }] }),
  component: MatchPage,
});
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      <Card className="mt-3 p-6">
        <div className="text-center text-xs text-muted-foreground">{match.league}</div>
        <div className="mt-3 grid grid-cols-3 items-center gap-4">
          ...
          <div className="text-center">
            {isLive ? (
              <>
                <div className="font-mono text-4xl font-extrabold tabular-nums text-live">{match.home_score} - {match.away_score}</div>
                <div className="mt-1 text-xs font-semibold text-live">{match.match_minute ?? 0}' LIVE</div>
              </>
            ) : isUpcoming ? (
              <Countdown to={match.kickoff_time} />
            ) : match.status === "ended" ? (
              <>
                <div className="font-mono text-4xl font-extrabold tabular-nums">{match.home_score} - {match.away_score}</div>
                <div className="mt-1 text-xs font-semibold text-muted-foreground">FT</div>
              </>
            ) : (
      ...
      <h2 className="mt-6 mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Markets</h2>
      <div className="grid gap-3 md:grid-cols-2">
        {Object.entries(oddsByMarket).map(([market, list]) => (
          <Card key={market} className="p-4">
            <div className="mb-2 text-sm font-semibold">{market}</div>
            <div className="grid grid-cols-3 gap-2">
              {list.map((o) => (
                <button key={o.selection} className="flex items-center justify-between rounded-md bg-secondary px-3 py-2 text-sm hover:bg-primary/20">
                  <span className="text-muted-foreground">{o.selection}</span>
                  <span className="font-mono font-semibold tabular-nums">{Number(o.value).toFixed(2)}</span>
                </button>
              ))}
            </div>
          </Card>
        ))}
        {odds.length === 0 && <Card className="p-6 text-center text-sm text-muted-foreground md:col-span-2">No odds posted yet.</Card>}
      </div>

      <h2 className="mt-6 mb-2 flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-secondary">
        {logo ? <img src={logo} alt={name} className="h-12 w-12 object-contain" /> : <span className="text-xl font-bold">{name.slice(0, 2).toUpperCase()}</span>}
      </div>
...
function SquadCard({ team, main, sub }: { team: string; main: Player[]; sub: Player[] }) {
  return (
    <Card className="p-4">
      ...
      <div className="text-xs uppercase tracking-wide text-muted-foreground">Starting XI</div>
      ...
      <div className="text-xs uppercase tracking-wide text-muted-foreground">Substitutes</div>
